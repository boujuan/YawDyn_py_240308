import os
import pandas as pd
from _stepresponse_analysis_original import plot_identified_yaw_maneuvers
...
def import_data_from_asc(file_path, sensor_name):
    ...
    data = data.loc[~data['time'].duplicated(keep=False)]  # Check for duplicates
...
def load_data(data_folder):
    ...
    yaw_dfs, wind_dfs, power_dfs, pitch_dfs, operation_state_dfs, controller_state_dfs = [], [], [], [], [], []
    for yaw_file, wind_file, power_file, pitch_file, operation_state_file in zip(yaw_file_names, wind_file_names, power_file_names, pitch_file_names, operation_state_file_names):
        ...
        try:
            ...
            print(f"Data from {yaw_file}, {wind_file}, {power_file}, {pitch_file}, and {operation_state_file} added to DataFrame list.")
        except FileNotFoundError:
            print(f"File not found: {yaw_path}, {wind_path}, {power_path}, {pitch_path}, or {operation_state_path}")
    ...
    if yaw_dfs and wind_dfs and power_dfs and pitch_dfs and operation_state_dfs and controller_state_dfs:
        ...
        combined_df = pd.concat([combined_yaw_df, combined_wind_df, combined_power_df, combined_pitch_df, combined_operation_state_df, combined_controller_state_df], axis=1)
...
if filtered_data is not None:
    ...
    #     combined_data[['yaw_3', 'yaw_4', 'yaw_5', 'yaw_6']],
    #     combined_data[['windspeed_3', 'windspeed_4', 'windspeed_5', 'windspeed_6']],
    #     path2dir_fig_base,
