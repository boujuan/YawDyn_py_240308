...
def analyze_yaw_maneuvers(yaw_data, wind_speed_data, path2dir_fig_base, date_range_total_str, resample_str):
    ...
    for turb_key in yaw_data.columns:
        yaw_filt = yaw_data[turb_key] # changed  turb key by :
        wind_speed_filt = wind_speed_data[f'windspeed_{turb_key.split("_")[-1]}']
        yaw_man, yaw_lengths, yaw_durations = find_yaw_maneuver(yaw_filt)
        
        # Separate data for cw and ccw maneuvers
        cw_indices = yaw_man[yaw_man == 'cw_start'].index
        ccw_indices = yaw_man[yaw_man == 'ccw_start'].index

        for start, stop, yaw_length, yaw_duration in zip(yaw_man[yaw_man == 'ccw_start'].index, yaw_man[yaw_man == 'ccw_stop'].index, yaw_lengths, yaw_durations):
            if start and stop:
                ...
                yaw_chunk_before = yaw_filt[start]
                yaw_chunk_after = yaw_filt[stop]
    ...
    for i, maneuver_type in enumerate(['cw', 'ccw', 'combined']):
        ...
        axs[i].scatter(subset['Yaw Length'], subset['Wind Speed Offset'], label=f'{maneuver_type} maneuvers', color='blue')
        ax2.scatter(subset['Yaw Length'], subset['Relative Wind Speed Change'], label=f'{maneuver_type} maneuvers (Relative Change)', color='green', marker='x')
        
        mean_value = subset['Wind Speed Offset'].mean()
        axs[i].axhline(mean_value, color='r', linestyle='--', label=f'Mean Offset: {mean_value:.2f}')
        
        # Fit and plot a trend line for Wind Speed Offset
        z = np.polyfit(subset['Yaw Length'], subset['Wind Speed Offset'], 1)
        p = np.poly1d(z)
        axs[i].plot(subset['Yaw Length'], p(subset['Yaw Length']), "r--", label=f'Trend line Offset: {z[0]:.2f}x + {z[1]:.2f}')
